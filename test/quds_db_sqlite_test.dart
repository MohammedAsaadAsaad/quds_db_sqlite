...
void main() {
  ...
  group('CRUD Operations', () {
    ...
    test('Query Builder - Where Clause and DeleteWhere', () async {
      ...
      final dCount =
          await provider.deleteWhere((n) => n.isImportant.equals(false));
  ...
  group('Performance and Migrations', () {
    test('Performance: Bulk Insert and Read 1,000 records', () async {
      ...
      expect(insertTime, lessThan(2000),
          reason: 'Inserting 1,000 records took too long ($insertTime ms)');
      ...
      expect(readTime, lessThan(1000),
          reason: 'Reading 1,000 records took too long ($readTime ms)');

      // Clear performance
      stopwatch.reset();
      await provider.clear();
      final clearTime = stopwatch.elapsedMilliseconds;
      expect(clearTime, lessThan(500),
          reason: 'Clearing 1,000 records took too long ($clearTime ms)');
    });
